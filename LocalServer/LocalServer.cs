...
namespace ASCOM.Ardufocus
{
    public static class Server
    {
        ...
        private static readonly string s_appId = "{2a552a6e-0a8a-47fa-a4c4-c5c1e4543bcb}";   // Our AppId
            ...
            foreach (FileInfo fi in d.GetFiles("*.dll"))
            {
                ...
                catch (Exception e)
                {
                    MessageBox.Show("Failed to load served COM class assembly " + fi.Name + " - " + e.Message,
                        "Ardufocus", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return false;
                }
        ...
        private static void ElevateSelf(string arg)
        {
            ...
            {
                MessageBox.Show("The Ardufocus was not " + (arg == "/register" ? "registered" : "unregistered") +
                    " because you did not allow it.", "Ardufocus", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString(), "Ardufocus", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
                ...
                switch (args[0].ToLower())
                {
                    ...
                    default:
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
                            "Ardufocus", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
        ...
        static void Main(string[] args)
        {
            ...
            if (!LoadComObjectAssemblies()) return;

            // Register and/or Unregister
            if (!ProcessArguments(args)) return;

            Mutex = new Mutex(
                true,
                String.Format("Local\\{0}", s_appId),
                out bool IsMutexOwner
            );

            const string Title = Common.Metadata.DriverDescription;
